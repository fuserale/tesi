...
for isubject = [1 2 3 5 6 7]
    ...
    for i=1:size_windows_sample-size_overlap_samples:m - size_windows_sample
        ...
        if classi(number_sample) == 1
           classi2(number_sample) = 1; 
        end
        if classi(number_sample) == 2
           classi2(number_sample) = 1; 
        end
        if classi(number_sample) == 3
           classi2(number_sample) = 2; 
        end
        number_sample = number_sample + 1;
    end
    %% Faccio cluster sul paziente escluso e poi LDA sui dati del cluster per allenare il secondo knn
    ALL = [F classi'];
    idx = kmeans(F,2,'MaxIter',10000,'Start','cluster','Replicates',5);
    
    [C1,~] = confusionmat(idx,classi2);
    ...
    for i=1:size_windows_sample-size_overlap_samples:m - size_windows_sample
        ...
        if classi3(number_sample) == 2
           classi3(number_sample) = 1; 
        end
        if classi3(number_sample) == 3
           classi3(number_sample) = 2; 
        end
        number_sample = number_sample + 1;
    end
    ...
    figure; gscatter(1:length(Y3),Y3(1,:),classi3);
    ...
    figure; gscatter(1:length(Y1),Y1(1,:),classi3);
...
%% Trova un modo per fare previsione su dato grezzo e feature
%% Prova clustering con dimensionalità ridotta
%% Aumenta parametri cluster