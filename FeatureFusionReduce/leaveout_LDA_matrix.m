...
for l = [1 2 3 5 6 7 8 9]
    ...
    for isubject = subject
        ...
        [row,col] = find (class == 1);
        NOFOG = F(col,:);
        LABEL_NOFOG = class(col)';
        [row,col] = find (class == 2);
        FOG = F(col,:);
        LABEL_FOG = class(col)';
        [row,col] = find (class == 3);
        PREFOG = F(col,:);
...
%         title(['LDA S' num2str(isubject,'%02d') ' #CLASS' num2str(K,'%01d')]);
    ...
    writetable(array2table(matrix_tr),[datadir '/matrix/matrix_leaveout_S' num2str(leaveout_subject,'%02d') '_second' num2str(windows,'%.1f') '_overlap' num2str(overlap,'%.1f') '.csv']);
    %     %% Fase di Classificazione
    %     my_knn = fitcknn(PR(:,1:2),PR(:,3),'NumNeighbors',5,'Standardize',1);
    %     %% Fase di Clustering
    %     % K-means
