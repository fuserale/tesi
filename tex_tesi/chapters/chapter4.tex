...
\node [mybox] (box){%
	...
	Quello che, dunque, si vuole progettare è un algoritmo automatizzato non supervisionato che, attraverso l'uso di accelerometri posizionati sul paziente, permetta di verificare l'esistenza di una nuova classe chiamata preFoG e che sia divisa da quelle di FoG e noFoG. Una volta dimostrato questo, si procede a trovare il miglior intervallo di suddivisione dei dati al fine di sviluppare un procedimento di riconoscimento automatico di etichette delle varie classi attraverso algoritmi di clustering, sostituendo cosí il dottore nella prima fase di test. Sfruttando tale intervallo, inoltre, si vuole tentare un primo approccio di classificazione per poter etichettare nuovi dati rispetto a quelli contenuti nel nostro dataset di allenamento.
	\end{minipage}
};
\end{tikzpicture}%
