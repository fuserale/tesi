...
for l = [1 2]
    ...
    for isubject = subject
        ...
        for r = 1:length(fileruns)
            ...
            ALLSENSOR = [ALLSENSOR;A];
            ALLTIME = [ALLTIME;TIME];
            ALLFREEZE = [ALLFREEZE;FREEZE];
        end
    end
    
    
    [row,~] = find (ALLFREEZE == 1);
    NOFOG = ALLSENSOR(row,:);
    NOFOGL = ALLFREEZE(row,:);
    [row,~] = find (ALLFREEZE == 2);
    FOG = ALLSENSOR(row,:);
    FOGL = ALLFREEZE(row,:);
    [row,~] = find (ALLFREEZE == 3);
    PREFOG = ALLSENSOR(row,:);
    PREFOGL = ALLFREEZE(row,:);
    
    
    X = 5;
    NOFOG2 = NOFOG(1:X*size(FOG),:);
    NOFOGL2 = NOFOGL(1:X*size(FOG),:);
    
    A = [NOFOG2;PREFOG;FOG];
    LABEL = [NOFOGL2;PREFOGL;FOGL];
    [m,n] = size(A);
    ...
    for k = 5:5:15
        ...
        for i = (Y+0.5):0.5:2
            ...
            for i=1:size_windows_sample-size_overlap_samples:m - size_windows_sample
                B = A(i:i+size_windows_sample-1,1:n);
                
                %time sample
                F(number_sample, 1) = number_sample;
                ...
                %is freezing?
                F(number_sample,131) = mode(A(i:i+size_windows_sample-1,n+1));
            ...
            P = array2table(F);
            writetable(P, ['../interval_2cl/S' num2str(leaveout_subject,'%02d') 'R01/overlap_' num2str(k,2) '/2cl_feature_sec' num2str(size_windows_sec*10,2) '_ov' num2str(k,2) '_' fileruns(r).name ]);
