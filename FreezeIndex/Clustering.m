...
for r = 1:length(fileruns)
    ...
    for q=1:4
        ...
        P = array2table(kidx);
        writetable(P, ['Clusters/kmeans_' dist_k '_' fileruns(r).name] );
        display(['kmeans_' dist_k '_' fileruns(r).name]);
    end
    ...
    P = array2table(nidx);
    writetable(P, ['Clusters/net_' fileruns(r).name] );
    display(['net_' fileruns(r).name]);
    ...
    P = array2table([fidx A(:,11)]);
    writetable(P, ['Clusters/cmeans_' fileruns(r).name] );
    display(['cmeans_' fileruns(r).name]);
end