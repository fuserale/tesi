...
for isubject = [1 2 3 5]
    ...
    for r = 1:length(fileruns)
        ...
        while i < m
            ...
            F(number_sample, 128) = svmn(B(:,1:3), length(B));
            F(number_sample, 129) = svmn(B(:,4:6), length(B));
            F(number_sample, 130) = svmn(B(:,7:9), length(B));
            %normalized signal magnitude area --> acceleration magnitude summed
            %over three axes normalized by the windows length
            F(number_sample, 131) = sman(B(:,1:3), length(B));
            F(number_sample, 132) = sman(B(:,4:6), length(B));
            F(number_sample, 133) = sman(B(:,7:9), length(B));
            ...
            F(number_sample,137) = energyn(B(:,1:3),length(B));
            F(number_sample,138) = energyn(B(:,4:6),length(B));
            F(number_sample,139) = energyn(B(:,7:9),length(B));
            %is freezing?
            F(number_sample,140) = mode(FREEZE(i:end_size-1,:));
        ...
        writetable(P, ['../../dataset/CSV/feature/dynamics/3cl_dynamics_' fileruns(r).name ]);
        display(['../../dataset/CSV/feature/dynamics/3cl_dynamics_' fileruns(r).name ]);
        F(:,:) = [];
    end
end
